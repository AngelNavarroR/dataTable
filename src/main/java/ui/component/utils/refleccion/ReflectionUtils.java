...
import static ui.component.utils.refleccion.StringUtil.EMPTY;
...
public class ReflectionUtils implements Serializable {
    ...
    private static final LRUCacheMap<String, Class<?>> classNameToClassCache = new LRUCacheMap<>(DEFAULT_MAX_CACHE_CAPACITY);

    private static final LRUCacheMap<String, Method> methodNameToMethodCache = new LRUCacheMap<>(DEFAULT_MAX_CACHE_CAPACITY);

    private static final LRUCacheMap<String, Field> fieldNameToFieldCache = new LRUCacheMap<>(DEFAULT_MAX_CACHE_CAPACITY);

    private static final LRUCacheMap<Class<?>, List<Field>> fieldClassCache = new LRUCacheMap<>(DEFAULT_MAX_CACHE_CAPACITY);

    /**
     * Gets the <code>Field</code> value from the field with the name specified
     * with the <code>fieldName</code> parameter from the Object instance
     * specified with the <code>object</code> parameter.
     * <br><br>
     * The Field which value should be fetched can also be declared as private,
     * packaged or protected and also from superclasses.
     *
     * @param object The object containing the desired Field.
     * @param clazz The <code>Class</code> to be fetched using
     * <code>object.getClass()</code> from the <code>object</code> parameter.
     * @param fieldName The name of the field which value should be fetched from
     * the Object specified with the <code>object</code> parameter.
    ...
    private static Field getField(final Object object, final Class<?> clazz, final String fieldName) throws ReflectionFailureException {
        ...
        } catch (NoSuchFieldException | SecurityException e) {/*not found, go on*/
     ...
     * the Object specified with the <code>object</code> parameter.
     * @param useGetter
     * @return The value of the <code>FieeException
     * @throws IllegalArgumentException
     */
        ...
        try {
            ...
            if (useGetter) {
                ...
                if (method != null) {
                    ...
                    Object methodResult = method.invoke(object, new Object());
                    return methodResult;
                }
            }
            ...
            return o;
        } catch (SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException | ReflectionFailureException e) {
    ...
    /**
     * Gets the <code>Field</code> value from the field with the name specified
     * with the <code>qualifiedFieldName</code> parameter. For example:
     * <code>"System.out"</code> returns the <code>{@link OutputStream}</code>
     * from the <code>{@link System}</code> class. If only a Class file is
     * requested with the <code>qualifiedFieldName</code> string, it will also
     * be returned.
     * <br><br>
     * The Field which value should be fetched can also be declared as private,
     * packaged or protected and also from superclasses.
     *
     * @param qualifiedFieldName the field name to be fetched. The qualified
     * field name must be described with the full package path the class name
     * and the name of the field as the last segment, all separated by a '.'
     * character.
     * @return The desired object instance or
     * <code>{@link java.lang.Class}</code>.
    ...
    /**
     * Puts the <code>value</code> to the field <code>fieldName</code> at the
     * given <code>object</code> instance.
     * <br><br>
     * This method is not applicable for setting static fields on a class. Only
     * object instanced can be feed into the <code>object</code> parameter.
     *
     * @param object Instancia del Objecto que contiene en field a modificar.
     * @param fieldName El nombre del field que se va modificar el valor.
     * @param value Valor a settear en el fiel <code>fieldName</code> parameter.
     * @throws data.refleccion.ReflectionFailureException
     * @throws IllegalArgumentException
     */
    ...
    /**
     * Executed the specified method in the specified class. Arguments will be
     * converted to numeric, boolean or string types.
     *
     * @param object The <code>Object</code> where the method specified with the
     * <code>methodName</code> parameter should be invoked.
     * @param methodName The method to be executed.
     * @param args The arguments for the method to be executed.
     * @return A <code>ExpressionNode</code> matching to the result of the cal
     * @throws scl.com.utils.ReflectionFailureExceptionled method.
     ...
     *
     * @param className The class name where a new Object instance should be
     * created for.
     ...
     * @param allowedReturnTypes All allowed field types. <code>null</code> for
     * all types.
     ...
     * @return The desired Field instance or <code>null</code> if no such field
     * could be found.
     */
     ...
     * For specifying the visibility, use the constants
     * <B>VISIBILITY_VISIBLE_ACCESSIBLE_ONLY</B>, <B>VISIBILITY_VISIBLE_ALL</B>.
    ...
    /**
     * filters duplicates from a <code>Method</code> or <code>Field</code>
     * array.
     *
     * @param values An Object that is a Method, a Field or provides a
     * <code>getName()</code> method.
     * @param compare
     * @return A new array instance without any duplicates.
     *
     * @throws RuntimeInvocationTargetException should not be happens but will
     * be possible if an array of Objects is specified which did not provides a
     * <code>getName()</code> method.
     */
        ...
        try {
            ...
            for (Object value : values) {
                ...
                if (value instanceof Method) {
                    ...
                    names.add((String) invokeMethod(value, "getName", new Object()));
                }
            }
            ...
            for (int i = 0; valuesIterator.hasNext(); i++) {
                ...
                if (value instanceof Method) {
                    ...
                    if (ListUtils.indexOf(names, (String) invokeMethod(value, "getName", new Object()), compare, UtilConstants.SEARCH_DEFAULT) == i) {
                        result.add((T) value);
                    }
                }
            }
    ...
    /**
     * Takes the <code>targetParameterTypes</code> and tries to convert the
     * <code>sourceValues</code> to a matching type. If a method requires an
     * <code>int</code> and a <code>{@link BigDecimal}</code> instance is given,
     * the <code>{@link BigDecimal}</code> instance will be converted to match
     * to the int.
     *
     * @param targetParameterTypes The parameter class types required.
     * @param sourceValues The values to be converted into the required
     * <code>targetParameterTypes</code>
     * @param force <code>true</code> means that the values array will also be
     * returned if it has not been completely converted. <code>false</code> will
     * return <code>null</code> if one value could not be identified and
     * converted.
     * @return All required object instances assignable to this ones given with
     * the <code>targetParameterTypes<7code> parameter.
     */
    private static Object[] createArguments(Class<?>[] targetParameterTypes, Object[] sourceValues, boolean force) {
        ...
        for (int j = 0; j < targetParameterTypes.length && j < sourceValues.length; j++) {
            ...
            if ((targetParameterTypes[j].equals(int.class) || targetParameterTypes[j].equals(Integer.class)) && CommonUtils.toNumber(sourceValues[j]) != null) {
                result[j] = CommonUtils.toNumber(sourceValues[j]).intValue();
                found++;
                continue;
                ...
                result[j] = CommonUtils.toNumber(sourceValues[j]).doubleValue();
                found++;
                continue;
                ...
                result[j] = CommonUtils.toNumber(sourceValues[j]).floatValue();
                found++;
                continue;
                ...
                result[j] = CommonUtils.toNumber(sourceValues[j]).longValue();
                found++;
                continue;
                ...
                result[j] = CommonUtils.toNumber(sourceValues[j]).shortValue();
                found++;
                continue;
            } //test for boolean 
                ...
                result[j] = String.valueOf(sourceValues[j]).charAt(0);
                found++;
                continue;
            } //test for String 
     ...
     * @param methodName The method to be executed.
     * @param args The arguments for the method to be executed.
     * @return A <code>ExpressionNode</code> matching to the result of the
     * called method.
     * @throws InvocationTargetException
     * @throws IllegalAccessException
     * @throws IllegalArgumentException
     ...
     * is a replacement for <code>Thread.currentThread().getStackTrace()</code>
     * existing with java 1.5
     *
    ...
    /**
     * Determines if the <code>invokingClass</code> and/or
     * <code>invokingMethod</code> is in the stack of the current thread. Anonym
     * classes are also assumed to the given <code>invokingClass</code>.
     *
     * @param invokingClass Class to be searched in the stack (required).
     * @param invokingMethod Method to be searched (can be null).
     * @return The location in the stack hierarchy or -1 if the class/method
     * could not be found in the thread stack.
     */
     ...
     *
     * @param first The first Class to be tested if the second class is
     * assignable to it.
     * @param second The second Class to be tested if it's assignable to the
     * first Class.
     * @return <code>true</code> if the second class is assignable to the first
     * class
     */
    ...
    /**
     * Copies all public fields and public getter/setter methods from the
     * <code>source</code> Object to the <code>target</code> Object.
     *
     ...
     * @throws scl.com.utils.ReflectionFailureException
     * @throws IllegalArgumentException
     */
    ...
    /**
     * gets the package without the classname, empty string if there is no
     * package.
     *
     * @param c The class where the package name should be extracted from.
    ...
    /**
     * Tries to load the <code>{@link Class}</code> specified with the
     * <code>className</code> parameter. The parameter should contain a
     * qualified class name.
     *
     * @param className The name of the <code>{@link Class}</code> to be loaded.
     * @return The desired <code>{@link Class}</code> or <code>null</code> if
     * the class could not be loaded. Never throws any kind of
    ...
    /**
     * Determina si la clase dada es un miembro de la clase (subclase).
     *
     * @param cls The {@link Class} para ser procesado.
     * @return <code>true</code> si tenemos una clase de miembro o
     * <code>false</code> caso contrario. Si la {@link Class} dada es
     * <code>null</code>, <code>false</code> es retornado.
     */
    ...
    /**
     * Tests if the both given classes are the same. It will not be detected if
     * both classes extends a common class or have the same interface.
     *
     ...
     * @return <code>true</code> if the classes equals and <code>false</code>
     * otherwise.
     */
     ...
     * is64Bit()
     *
     * Determina si es un ambiente de 64 bit
     *
     * @return
     */
    ...
    public static String[] getNamesPropertys(Object cols, String... ignoreCols) {
        List<Field> fields = getFields(cols.getClass(), 1);
        List<String> names = new ArrayList<>();
        for (Field field : fields) {
            boolean ignore = false;
            for (int i = 0; i < ignoreCols.length; i++) {
                if (ignoreCols[i].equals(field.getName())) {
                    ignore = true;
                }
            }
            if (!ignore) {
    ...
    /**
     * Determines a two digit version number. The java version 1.5.0_11 simply
     * returns 15, the version 1.4.2 returns 14 and so on.
     *
     * @return The two digit version number or -1 if no version could be
     * determined. Never throws any kind of <code>{@link Exception}</code>.
     */
    ...
    /**
     * Does a <code>Thread.sleep</code> but without having these nasty exception
     * handling with the {@link InterruptedException}. The
     * {@link InterruptedException} gets simple be logged.
     *
     * @param time
     */
    public static void sleepSilent(int time) {
        try {
            ...
            LOG.log(Level.WARNING, "Sleep has been interrupted", e);
        }
    }
    ...
    protected Class getDomainClass() {
        ...
        domainClass = (Class<?>) parametrizedType.getActualTypeArguments()[0];
        return domainClass;
    }
